...
const domain_2 = require("../../../src/domain");
const domain_3 = require("../../../src/domain");
const domain_4 = require("../../../src/domain");
...
describe('Application Layer - Service: CouponService', () => {
    ...
    let mockLogger;
    beforeEach(() => {
        mockCouponRepository = new helpers_1.MockCouponRepository();
        mockLogger = {
            info: jest.fn(),
            warn: jest.fn(),
            error: jest.fn(),
        ...
        couponService = new application_1.CouponService(mockCouponRepository, mockLogger);
    });
    afterEach(() => {
    ...
    describe('generateCoupon', () => {
        it('should generate coupon with valid value', async () => {
            const value = domain_1.Balance.fromDollars(50);
        ...
        it('should generate unique codes', async () => {
            const value = domain_1.Balance.fromDollars(25);
        ...
        it('should save coupon to repository', async () => {
            const value = domain_1.Balance.fromDollars(100);
            const coupon = await couponService.generateCoupon(value);
        ...
        it('should handle small values ($1)', async () => {
            const value = domain_1.Balance.fromDollars(1);
            const coupon = await couponService.generateCoupon(value);
            expect(coupon.getValue().getCents()).toBe(100);
        });
        it('should handle large values ($1000)', async () => {
            const value = domain_1.Balance.fromDollars(1000);
            const coupon = await couponService.generateCoupon(value);
    ...
    describe('redeemCoupon', () => {
        it('should redeem valid unactivated coupon', async () => {
            const code = domain_2.CouponCode.generate();
            const value = domain_1.Balance.fromDollars(50);
            const userId = domain_3.UserId.create(123456);
        ...
        it('should throw CouponNotFoundError for non-existent coupon', async () => {
            const code = domain_2.CouponCode.create('INVALID-CODE');
            const userId = domain_3.UserId.create(123456);
            await expect(couponService.redeemCoupon(code, userId)).rejects.toThrow(domain_4.CouponNotFoundError);
            await expect(couponService.redeemCoupon(code, userId)).rejects.toThrow('Coupon not found: INVALID-CODE');
        });
        it('should throw InvalidCouponError for already activated coupon', async () => {
            const code = domain_2.CouponCode.generate();
            const value = domain_1.Balance.fromDollars(50);
            const userId1 = domain_3.UserId.create(111);
            const userId2 = domain_3.UserId.create(222);
            await mockCouponRepository.create(code, value);
            await couponService.redeemCoupon(code, userId1);
            await expect(couponService.redeemCoupon(code, userId2)).rejects.toThrow(domain_4.InvalidCouponError);
            await expect(couponService.redeemCoupon(code, userId2)).rejects.toThrow('Coupon already activated');
        });
        it('should handle redemption by different users', async () => {
            const code1 = domain_2.CouponCode.generate();
            const code2 = domain_2.CouponCode.generate();
            const value = domain_1.Balance.fromDollars(25);
            const user1 = domain_3.UserId.create(111);
            const user2 = domain_3.UserId.create(222);
    ...
    describe('integration scenarios', () => {
        ...
        it('should handle multiple coupons generation and redemption', async () => {
            const coupons = await Promise.all([
                couponService.generateCoupon(domain_1.Balance.fromDollars(10)),
                couponService.generateCoupon(domain_1.Balance.fromDollars(20)),
                couponService.generateCoupon(domain_1.Balance.fromDollars(30)),
            ]);
            expect(coupons).toHaveLength(3);
            expect(mockCouponRepository.getAll()).toHaveLength(3);
            await couponService.redeemCoupon(coupons[0].getCode(), domain_3.UserId.create(111));
            await couponService.redeemCoupon(coupons[1].getCode(), domain_3.UserId.create(222));
    ...
    describe('error handling', () => {
        it('should not activate coupon if save fails', async () => {
            ...
            const errorRepo = {
                findByCode: jest
                    .fn()
                    .mockResolvedValue(helpers_1.MockFactory.createCoupon({ code: code.getValue() })),
                save: jest.fn().mockRejectedValue(new Error('Database error')),
                create: jest.fn(),
                activate: jest.fn(),
            };
            const errorService = new application_1.CouponService(errorRepo, mockLogger);
