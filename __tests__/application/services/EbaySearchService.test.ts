...
import { EbayBrowseApiClient } from '../../../src/infrastructure/ebay/EbayBrowseApiClient';
import { EbayFindingApiClient } from '../../../src/infrastructure/ebay/EbayFindingApiClient';
import { ILogger } from '../../../src/infrastructure/logging/Logger';

describe('Application Layer - Service: EbaySearchService', () => {
  let ebaySearchService: EbaySearchService;
  let mockBrowseClient: jest.Mocked<EbayBrowseApiClient>;
  let mockFindingClient: jest.Mocked<EbayFindingApiClient>;
  ...
  beforeEach(() => {
    ...
    ebaySearchService = new EbaySearchService(mockBrowseClient, mockFindingClient, mockLogger);
  });
  ...
  describe('search - ACTIVE config (Browse API)', () => {
    it('should search using Browse API for ACTIVE config', async () => {
      // Arrange
      const partNumbers = [PartNumber.create('PART-001')];
      ...
      expect(mockBrowseClient.searchActiveItems).toHaveBeenCalledWith(
        'PART-001',
        expect.any(Object)
      );
    ...
    it('should handle multiple part numbers with ACTIVE config', async () => {
      // Arrange
      const partNumbers = [
        PartNumber.create('PART-001'),
        PartNumber.create('PART-002'),
        PartNumber.create('PART-003'),
      ];
      const configKey = SearchConfigKey.create('ACTIVE');
  ...
  describe('search - SOLD config (Finding API)', () => {
    it('should search using Finding API for SOLD config', async () => {
      // Arrange
      const partNumbers = [PartNumber.create('SOLD-PART')];
      ...
      expect(mockFindingClient.searchCompletedItems).toHaveBeenCalledWith(
        'SOLD-PART',
        expect.any(Object)
      );
    ...
    it('should handle multiple part numbers with SOLD config', async () => {
      // Arrange
      const partNumbers = [PartNumber.create('SOLD-001'), PartNumber.create('SOLD-002')];
      const configKey = SearchConfigKey.create('SOLD');
    ...
    it('should return not found result when Finding API returns empty array', async () => {
      // Arrange
      const partNumbers = [PartNumber.create('NO-SOLD')];
      const configKey = SearchConfigKey.create('SOLD');
  ...
  describe('search - ENDED config (Finding API)', () => {
    it('should search using Finding API for ENDED config', async () => {
      // Arrange
      const partNumbers = [PartNumber.create('ENDED-PART')];
  ...
  describe('search - mixed results', () => {
    it('should handle mix of found and not found items', async () => {
      // Arrange
      const partNumbers = [
        PartNumber.create('FOUND-1'),
        PartNumber.create('NOT-FOUND'),
        PartNumber.create('FOUND-2'),
      ];
      const configKey = SearchConfigKey.create('ACTIVE');
  ...
  describe('search - error handling', () => {
    it('should propagate error from Browse API', async () => {
      ...
      // Act & Assert
      await expect(ebaySearchService.search(partNumbers, configKey)).rejects.toThrow(
        'eBay API Error'
      );
    });
    ...
    it('should fail all searches if one fails', async () => {
      // Arrange
      const partNumbers = [
        PartNumber.create('GOOD-1'),
        PartNumber.create('ERROR'),
        PartNumber.create('GOOD-2'),
      ];
      const configKey = SearchConfigKey.create('ACTIVE');
  ...
  describe('search - edge cases', () => {
    ...
    it('should handle single part number', async () => {
      // Arrange
      const partNumbers = [PartNumber.create('SINGLE')];
      const configKey = SearchConfigKey.create('SOLD');
    ...
    it('should use first item when API returns multiple items', async () => {
      // Arrange
      const partNumbers = [PartNumber.create('MULTI')];
      const configKey = SearchConfigKey.create('ACTIVE');
  ...
  describe('search - different currencies', () => {
    it('should handle different price currencies', async () => {
      // Arrange
      const partNumbers = [
        PartNumber.create('USD-ITEM'),
        PartNumber.create('EUR-ITEM'),
        PartNumber.create('GBP-ITEM'),
      ];
      const configKey = SearchConfigKey.create('ACTIVE');
