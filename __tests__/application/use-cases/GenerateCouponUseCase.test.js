...
const GenerateCouponUseCase_1 = require("../../../src/application/use-cases/GenerateCouponUseCase");
...
describe('Application Layer - Use Case: GenerateCouponUseCase', () => {
    ...
    beforeEach(() => {
        ...
        useCase = new GenerateCouponUseCase_1.GenerateCouponUseCase(mockCouponService, mockLogger);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('execute - successful generation', () => {
        it('should generate coupon with valid value', async () => {
            ...
            const mockCoupon = MockFactory_1.MockFactory.createCoupon({
                code: 'ABCD-1234-EFGH',
                valueCents: 5000,
            });
            ...
            expect(mockCouponService.generateCoupon).toHaveBeenCalledWith(expect.objectContaining({
                getCents: expect.any(Function),
            }));
            const balanceArg = mockCouponService.generateCoupon.mock.calls[0][0];
    ...
    describe('execute - validation', () => {
        it('should throw error for zero value', async () => {
            ...
            // Act & Assert
            await expect(useCase.execute({ valueInDollars })).rejects.toThrow('Coupon value must be positive');
    ...
    describe('execute - coupon code uniqueness', () => {
        it('should generate different codes for multiple coupons', async () => {
            // Arrange
            const mockCoupon1 = MockFactory_1.MockFactory.createCoupon({
                code: 'CODE-0001',
                valueCents: 5000,
            });
            const mockCoupon2 = MockFactory_1.MockFactory.createCoupon({
                code: 'CODE-0002',
                valueCents: 5000,
            });
            const mockCoupon3 = MockFactory_1.MockFactory.createCoupon({
                code: 'CODE-0003',
                valueCents: 5000,
            });
    ...
    describe('execute - error handling', () => {
        ...
        it('should propagate unique constraint error', async () => {
            // Arrange
            const valueInDollars = 50;
            mockCouponService.generateCoupon.mockRejectedValue(new Error('UNIQUE constraint failed: coupons.code'));
            // Act & Assert
    ...
    describe('execute - return value structure', () => {
        it('should return object with code and value', async () => {
            ...
            expect(result.code).toBeInstanceOf(CouponCode_1.CouponCode);
            expect(result.value).toBeInstanceOf(Balance_1.Balance);
        });
    });
});
