...
describe('Domain Layer - Value Object: UserId', () => {
    describe('create', () => {
        ...
        it('should throw error for negative user ID', () => {
            expect(() => UserId_1.UserId.create(-1)).toThrow('UserId must be a positive integer');
        });
        it('should throw error for zero user ID', () => {
            expect(() => UserId_1.UserId.create(0)).toThrow('UserId must be a positive integer');
        });
        it('should throw error for non-integer user ID', () => {
            expect(() => UserId_1.UserId.create(123.45)).toThrow('UserId must be a positive integer');
        });
    });
    describe('equals', () => {
        it('should return true for equal user IDs', () => {
            const userId1 = UserId_1.UserId.create(123456);
            const userId2 = UserId_1.UserId.create(123456);
            expect(userId1.equals(userId2)).toBe(true);
        });
        it('should return false for different user IDs', () => {
            const userId1 = UserId_1.UserId.create(123456);
            const userId2 = UserId_1.UserId.create(654321);
