...
const EbaySearchConfigFactory_1 = require("../../../src/infrastructure/ebay/EbaySearchConfigFactory");
...
describe('Infrastructure Layer - eBay: EbaySearchConfigFactory', () => {
    describe('create - ACTIVE config', () => {
        it('should create Browse API config for ACTIVE', () => {
            ...
            const config = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(configKey);
            expect(config).toBeDefined();
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isBrowseConfig(config)).toBe(true);
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isFindingConfig(config)).toBe(false);
        });
    ...
    describe('create - SOLD config', () => {
        it('should create Finding API config for SOLD', () => {
            ...
            const config = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(configKey);
            expect(config).toBeDefined();
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isFindingConfig(config)).toBe(true);
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isBrowseConfig(config)).toBe(false);
        });
    ...
    describe('isBrowseConfig type guard', () => {
        ...
        it('should narrow type to BrowseApiConfig', () => {
            const configKey = SearchConfigKey_1.SearchConfigKey.create('ACTIVE');
            const config = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(configKey);
            if (EbaySearchConfigFactory_1.EbaySearchConfigFactory.isBrowseConfig(config)) {
    ...
    describe('isFindingConfig type guard', () => {
        ...
        it('should narrow type to FindingApiConfig', () => {
            const configKey = SearchConfigKey_1.SearchConfigKey.create('SOLD');
            const config = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(configKey);
            if (EbaySearchConfigFactory_1.EbaySearchConfigFactory.isFindingConfig(config)) {
                // TypeScript should narrow type here
                expect(config.itemFilter).toBeDefined();
    ...
    describe('config differences', () => {
        it('SOLD and ENDED should have different filters', () => {
            const soldKey = SearchConfigKey_1.SearchConfigKey.create('SOLD');
            const endedKey = SearchConfigKey_1.SearchConfigKey.create('ENDED');
            const soldConfig = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(soldKey);
            const endedConfig = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(endedKey);
        ...
        it('ACTIVE config should be distinct from Finding configs', () => {
            const activeKey = SearchConfigKey_1.SearchConfigKey.create('ACTIVE');
            const soldKey = SearchConfigKey_1.SearchConfigKey.create('SOLD');
            const activeConfig = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(activeKey);
            const soldConfig = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(soldKey);
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isBrowseConfig(activeConfig)).toBe(true);
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isFindingConfig(soldConfig)).toBe(true);
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isBrowseConfig(soldConfig)).toBe(false);
            expect(EbaySearchConfigFactory_1.EbaySearchConfigFactory.isFindingConfig(activeConfig)).toBe(false);
        });
    });
    describe('config consistency', () => {
        it('should create same config for same key', () => {
            const key1 = SearchConfigKey_1.SearchConfigKey.create('ACTIVE');
            const key2 = SearchConfigKey_1.SearchConfigKey.create('ACTIVE');
            const config1 = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(key1);
            const config2 = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(key2);
    ...
    describe('all config keys support', () => {
        it('should support all SearchConfigKey values', () => {
            ...
            keys.forEach((key) => {
                const configKey = SearchConfigKey_1.SearchConfigKey.create(key);
                expect(() => EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(configKey)).not.toThrow();
            });
        });
        it('should create config for each key type', () => {
            const activeConfig = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('ACTIVE'));
            const soldConfig = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('SOLD'));
            const endedConfig = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('ENDED'));
    ...
    describe('type guard mutual exclusivity', () => {
        it('config cannot be both Browse and Finding', () => {
            const configs = [
                EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('ACTIVE')),
                EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('SOLD')),
                EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('ENDED')),
            ];
            configs.forEach((config) => {
                const isBrowse = EbaySearchConfigFactory_1.EbaySearchConfigFactory.isBrowseConfig(config);
                const isFinding = EbaySearchConfigFactory_1.EbaySearchConfigFactory.isFindingConfig(config);
    ...
    describe('config structure validation', () => {
        it('BrowseApiConfig should have required properties', () => {
            const config = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('ACTIVE'));
        ...
        it('FindingApiConfig should have required properties', () => {
            const config = EbaySearchConfigFactory_1.EbaySearchConfigFactory.create(SearchConfigKey_1.SearchConfigKey.create('SOLD'));
