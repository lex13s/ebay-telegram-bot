...
const ExcelReportGenerator_1 = require("../../../src/infrastructure/excel/ExcelReportGenerator");
...
describe('Infrastructure Layer - Excel: ExcelReportGenerator', () => {
    ...
    beforeEach(() => {
        ...
        generator = new ExcelReportGenerator_1.ExcelReportGenerator(mockLogger);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('generate - basic functionality', () => {
        it('should generate Excel buffer from search results', async () => {
            const results = [helpers_1.MockFactory.createSearchResult({ partNumber: 'PART-001' })];
    ...
    describe('generate - headers', () => {
        it('should create correct column headers', async () => {
            const results = [helpers_1.MockFactory.createSearchResult({ partNumber: 'TEST' })];
            const buffer = await generator.generate(results);
            const workbook = new exceljs_1.default.Workbook();
            await workbook.xlsx.load(buffer);
    ...
    describe('generate - data rows', () => {
        it('should add single result as data row', async () => {
            const results = [
                helpers_1.MockFactory.createSearchResult({
                    partNumber: 'PART-123',
                    title: 'Test Item',
                    priceValue: '99.99',
                    priceCurrency: 'USD',
        ...
        it('should add multiple results as data rows', async () => {
            const results = [
                ...
                helpers_1.MockFactory.createSearchResult({ partNumber: 'PART-003' }),
            ];
            const buffer = await generator.generate(results);
            const workbook = new exceljs_1.default.Workbook();
            await workbook.xlsx.load(buffer);
        ...
        it('should handle "Not Found" results', async () => {
            const results = [helpers_1.MockFactory.createSearchResultNotFound('NOT-FOUND')];
            const buffer = await generator.generate(results);
            const workbook = new exceljs_1.default.Workbook();
            await workbook.xlsx.load(buffer);
            const worksheet = workbook.worksheets[0];
        ...
        it('should handle mix of found and not found results', async () => {
            const results = [
                helpers_1.MockFactory.createSearchResult({ partNumber: 'FOUND-001' }),
                helpers_1.MockFactory.createSearchResultNotFound('NOT-FOUND'),
                helpers_1.MockFactory.createSearchResult({ partNumber: 'FOUND-002' }),
            ];
            const buffer = await generator.generate(results);
            const workbook = new exceljs_1.default.Workbook();
            await workbook.xlsx.load(buffer);
    ...
    describe('generate - large datasets', () => {
        it('should handle 100 results', async () => {
            const results = Array.from({ length: 100 }, (_, i) => helpers_1.MockFactory.createSearchResult({ partNumber: `PART-${i}` }));
            const buffer = await generator.generate(results);
            const workbook = new exceljs_1.default.Workbook();
            await workbook.xlsx.load(buffer);
            const worksheet = workbook.worksheets[0];
        ...
        it('should handle 1000 results', async () => {
            const results = Array.from({ length: 1000 }, (_, i) => helpers_1.MockFactory.createSearchResult({ partNumber: `PART-${i}` }));
            const buffer = await generator.generate(results);
            expect(buffer).toBeInstanceOf(Buffer);
    ...
    describe('generate - error handling', () => {
        it('should log error and throw on generation failure', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to generate Excel report', expect.any(Error));
        });
    });
    describe('buffer properties', () => {
        ...
        it('should generate different buffers for different data', async () => {
            const results1 = [helpers_1.MockFactory.createSearchResult({ partNumber: 'PART-1' })];
            const results2 = [helpers_1.MockFactory.createSearchResult({ partNumber: 'PART-2' })];
